...
import { SkyvernRequestError, portalPolicySchema, type PortalPolicy, type PortalSnapshot, type SkyvernBrowser } from './skyvern-browser.js';
...
export class PortalAccess {
  readonly policies: Map<string, PortalPolicy>;
  constructor(readonly store: BennyStore, readonly browser: SkyvernBrowser, policies: PortalPolicy[],
    ...
    for (const raw of policies) {
      const p = portalPolicySchema.parse(raw);
  ...
  async begin(a: Account, tx: PoolClient, provider: string): Promise<void> {
    ...
    enqueue(a, `Connect ${provider} with Skyvern using this private 10-minute link:\n${this.origin}/benny/portal#${ticket}\nYou control the remote browser for sign-in and MFA. Do not text credentials. After sign-in, I’ll close it, restore the saved login and check the account. You must confirm here before further reads. Automated portal writes are not enabled.`, this.now(), false,
    ...
    try {
      switch (b.phase) {
        ...
        case 'save_login': {
          ...
          b.profile = profile; delete b.session; delete b.profileName; b.phase = 'restore'; break;
        }
        case 'restore': b.session = (await this.browser.create(job.policy.readUrl, b.profile)).id; b.phase = 'read'; break;
        case 'read': {
