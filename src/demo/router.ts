...
function systemPrompt(state: DemoState, pending?: string): string {
  ...
  return [
    ...
    `- You DO have the plan, the network directory and the person's household. NEVER say you can't search, find, look up, or check something — if it maps to an action above, choose that action instead of "say".`,
    `- NEVER claim you booked, filed, sent, paid, or checked anything unless the state below proves it.`,
    `- If they ask for something genuinely outside these benefits (e.g. calling a specific dentist), say plainly what you cannot do and offer the closest action you CAN — never just chat past it.`,
    `- Only use {"do":"menu"} when they explicitly ask what you can do; never as a fallback.`,
    ``,
